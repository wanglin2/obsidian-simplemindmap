<template>
  <Sidebar ref="sidebar" :title="$t('nodeIconSidebar.title')">
    <div class="nodeIconContent " :class="{ isDark: isDark }">
      <div class="boxHeader">
        ...
        <el-select
          ...
          v-model="currentType"
          :placeholder="$t('import.pleaseSelect')"
...
export default {
  ...
  created() {
    ...
    this.$root.$bus.$on('showNodeIcon', this.handleShowNodeIcon)
  },
  beforeDestroy() {
    this.$root.$bus.$off('node_active', this.handleNodeActive)
    this.$root.$bus.$off('showNodeIcon', this.handleShowNodeIcon)
  },
  methods: {
