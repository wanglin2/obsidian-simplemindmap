...
export default {
  ...
  created() {
    ...
    this.$root.$bus.$on('showNodeTag', this.hide)
  },
  beforeDestroy() {
    ...
    this.$root.$bus.$off('showNodeTag', this.hide)
  },
  mounted() {
    this.mindMap.el.parentNode.append(this.$refs.elRef)
  },
  methods: {
    onNodeTagClick(node, tag, index, el) {
      ...
      const boxWidth = 260
      const boxHeight = 152
      let left = rect.x + rect.width / 2 - boxWidth / 2 - elRect.left
      let top = rect.y + rect.height - elRect.top
      if (left < 0) {
        left = 0
      }
      if (left + boxWidth > elRect.width) {
        left = elRect.width - boxWidth
      }
      if (top < 0) {
        top = 0
      }
      if (top + boxHeight > elRect.height) {
        top = rect.y - elRect.top - boxHeight
      }
