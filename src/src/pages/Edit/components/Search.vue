...
export default {
  ...
  created() {
    this.$root.$bus.$on('show_search', this.showSearch)
  ...
  beforeDestroy() {
    this.$root.$bus.$off('show_search', this.showSearch)
  ...
  methods: {
    ...
    showSearch() {
      this.$root.$bus.$emit('closeSideBar')
      this.show = true
    ...
    setSearchResultListHeight() {
      this.searchResultListHeight = this.mindMap.elRect.height - 267 - 24
    },
