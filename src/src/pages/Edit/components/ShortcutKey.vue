<template>
  <Sidebar ref="sidebar" :title="$t('shortcutKey.title')">
    <div class="shortcutKeyContent" :class="{ isDark: isDark }">
      <div v-for="item in shortcutKeyList" :key="item.type">
        <div class="title" style="margin-top: 12px;">{{ item.type }}</div>
...
export default {
  ...
  computed: {
    ...
    shortcutKeyList() {
      return shortcutKeyList[this.$i18n.locale] || shortcutKeyList.en
    }
  },
  watch: {
    activeSidebar(val) {
